...
def verify_db_index(table, index):
    try:
        verify_db_table(table)
...
def verify_db_machine_state():
    try:
        verify_db_index('machine_state', 'machine_id')
    except RqlRuntimeError:
...
def find_machine_state():
    ...
    """
    try:
        verify_db_machine_state()  # First make sure our DB tables are all in order.
